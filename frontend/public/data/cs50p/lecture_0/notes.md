...
> Source: [CS50P 2022 — Lecture 0](https://cs50.harvard.edu/python/2022/notes/0/#lecture-0)
...
- In the terminal window, you can execute commands. To run this program, move your cursor to the bottom of the screen, clicking in the terminal window. Next to the dollar sign, type `python hello.py` and press the enter key on your keyboard.
...
- Imagine in our `hello.py` program that we accidentally typed `print("hello, world"` — notice that we missed the final `)` required by the compiler. If you purposefully make this mistake, the compiler will output an error in the terminal window!
...
```
  ...
  Notice that this equal `=` sign in the middle has a special role in programming. This equal sign literally assigns what is on the right to what is on the left. Therefore, the value returned by `input("What's your name? ")` is assigned to `name`.

- The program will return `hello, world` in the terminal window regardless of what the user types.
...
- You can add comments to your program to see what it is doing:
...
```
  ...
  The output in the terminal, if we typed "David", would be `hello, David`. Success.

## Strings and Parameters

- A string, known as a `str` in Python, is a sequence of text.
- Functions take arguments that influence their behavior. If we look at the documentation for [`print`](https://docs.python.org/3/library/functions.html#print) you'll learn that the print function automatically includes a piece of code `end='\n'`. This `\n` indicates that the print function will automatically create a line break when run. The print function takes an argument called `end` and the default is to create a new line.
...
- First, you could simply change the quotes to single quotation marks.
- Another, more commonly used approach: `print("hello, \"friend\"")`. The backslashes tell the compiler that the following character should be considered a quotation mark in the string and avoid a compiler error.
...
```
  ...
  Notice the `f` in `print(f"hello, {name}")`. This `f` is a special indicator for Python to treat this string a special way. Expect that you will be using this style of strings quite frequently in this course.
...
- It turns out that built into strings is the ability to remove whitespace from a string. Using the method `strip` on `name` as `name = name.strip()` will strip all the whitespace on the left and right of the user's input:
...
- Opening up VS Code, we can type `code calculator.py` in the terminal. First, we can declare a few variables:
...
```
  ...
  Naturally, when we run `python calculator.py` we get the result `3`. We can make this more interactive using the `input` function:
...
- Because your input comes into the compiler as text, it is treated as a string. We therefore need to convert this input from a string to an integer:
...
- We can further improve our program:
...
```
  ...
  This illustrates that you can run functions on functions. The inner function is run first, and then the outer one. First, the `input` function is run. Then, the `int` function.
...
- Regardless of what approach you take, remember that your code must be readable. You should use comments to give yourself and others clues about what your code is doing.
...
- Let's imagine you want to round the total to the nearest integer:
...
- What if we wanted to format the output of long numbers? For example, rather than `1000`, you may wish to see `1,000`:
...
```
  ...
  That `print(f"{z:,}")` creates a scenario where the outputted `z` will include commas, such as `1,000` or `2,500`.
...
```
  ...
  When inputting `2` as x and `3` as y, the result z is `0.6666666666`, seemingly going on to infinity.

- Let's imagine that we want to round this:
...
- We could also use an f-string to format the output:
...
- We can create our own function called `hello`:
...
```
  ...
  Notice that everything under `def hello()` is indented. Python uses indentation to understand what is part of the function above.
...
```
  ...
  When you call `hello(name)` the computer passes `name` into the `hello` function as `to`. This is how we pass values into functions.

- We can add a default value to `hello`:
...
- We can move our function down, but we need to tell the compiler that we have a `main` function and a separate `hello` function. The following modification will call the `main` function and restore our program to working order:
...
- You can imagine many scenarios where you don't just want a function to perform an action but also to return a value back to the main function. This "passing back" of a value we call a `return` value.
